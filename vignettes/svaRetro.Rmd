...
```{r, include=TRUE,results="markup",message=TRUE,warning=TRUE}
library(TxDb.Hsapiens.UCSC.hg19.knownGene)
library(dplyr)
hg19.genes <- TxDb.Hsapiens.UCSC.hg19.knownGene

RT <- rtDetect(RT_gr, hg19.genes, maxgap = 10, minscore = 0.8)
# Diagnostics: show the structure of RT for debugging
message("RT class: ", paste(class(RT), collapse = ", "))
message("RT is.list: ", is.list(RT))
if (is.list(RT)) {
  message("RT names: ", paste(names(RT), collapse = ", "))
} else {
  message("RT length: ", length(RT))
}
```
...
if (!is.null(rt_chr_prefix) && length(rt_chr_prefix) > 0) {
  GenomeInfoDb::seqlevelsStyle(rt_chr_prefix) <- "UCSC"
  pairs <- breakpointgr2pairs(rt_chr_prefix)
  pairs
} else {
  stop(
    "RT$SKA3 is NULL - rtDetect did not find SKA3. ",
    "RT class: ", paste(class(RT), collapse = ", "), "; ",
    "is.list: ", is.list(RT), "; ",
    if (is.list(RT)) {
      paste("names:", paste(names(RT), collapse = ", "))
    } else {
      paste("length:", length(RT))
    }
  )
}
```

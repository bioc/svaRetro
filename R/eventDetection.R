...
rtDetect <- function(gr, genes, maxgap = 100, minscore = 0.4) {
  ...
  # Harmonize seqinfo between gr and exons to avoid mismatch errors (e.g., MT length)
  # Prune exons to only include seqlevels present in gr
  common_seqs <- intersect(GenomeInfoDb::seqlevels(gr), GenomeInfoDb::seqlevels(exons))
  exons <- GenomeInfoDb::keepSeqlevels(exons, common_seqs, pruning.mode = "coarse")
  # Sync complete seqinfo (seqlengths, genome, isCircular) from exons to gr
  # This ensures findOverlaps has fully compatible seqinfo between objects
  exons_si <- GenomeInfoDb::seqinfo(exons)
  GenomeInfoDb::seqlengths(gr)[common_seqs] <- GenomeInfoDb::seqlengths(exons_si)[common_seqs]
  GenomeInfoDb::isCircular(gr)[common_seqs] <- GenomeInfoDb::isCircular(exons_si)[common_seqs]
  GenomeInfoDb::genome(gr)[common_seqs] <- GenomeInfoDb::genome(exons_si)[common_seqs]

  # find exon-SV overlaps:
  hits.start <- findOverlaps(gr, exons,
    maxgap = maxgap, type = "start",
    ignore.strand = TRUE
  )
  hits.end <- findOverlaps(partner(gr), exons,
    maxgap = maxgap, type = "end",
    ignore.strand = TRUE
  )
  ...
  same.tx <- vapply(
    Reduce(
      ...
      list(
        mcols(exons)[hits$subjectHits.x, "tx_id"],
  ...
  if (nrow(hits.tx) == 0 & nrow(hits.insSite) == 0) {
    ...
    l_gene_symbol <- unique(c(
      as.character(unlist(rt.gr$gene_symbol)),
    ...
    insSite.gr.idx <- lapply(
      l_gene_symbol,
      function(gs) {
        vapply(
          insSite.gr$gene_symbol,
          function(x) gs %in% x,
          logical(1)
        )
      }
